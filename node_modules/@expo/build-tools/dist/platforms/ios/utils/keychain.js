...
class Keychain {
    constructor(logger) {
        this.created = false;
        this.logger = logger;
        this.keychainPath = path_1.default.join(os_1.default.tmpdir(), `turtle-v2-${uuid_1.v4()}.keychain`);
        this.keychainPassword = uuid_1.v4();
    }
    get data() {
        return {
    ...
    async destroy(keychainPath) {
        ...
        await fastlane_1.default(['run', 'delete_keychain', `keychain_path:${keychainToDeletePath}`]);
    }
    ...
    async findIdentitiesByTeamId(teamId) {
        const { output } = await turtle_spawn_1.default('security', ['find-identity', '-v', '-s', `(${teamId})`, this.keychainPath], {
            stdio: 'pipe',
        });
        return output.join('');
    }
