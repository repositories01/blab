...
async function _checkNpmVersionAsync(projectRoot) {
  try {
    ...
    const npmVersionResponse = await (0, _spawnAsync().default)('npm', ['--version']);
...
async function _checkWatchmanVersionAsync(projectRoot) {
  ...
  const watchmanVersion = await Watchman().unblockAndGetVersionAsync(projectRoot); // If we can't get the watchman version, `getVersionAsync` will return `null`
...
async function _validateNodeModulesAsync(projectRoot) {
  ...
  try {
    const result = _fsExtra().default.statSync(_path().default.join(nodeModulesPath, 'node_modules'));
...
async function getExpoSdkStatus(projectRoot) {
  ...
  try {
    ...
    const mainFilePath = _path().default.join(projectRoot, pkg.main);

    const mainFile = await _fsExtra().default.readFile(mainFilePath, 'utf8'); // TODO: support separate .ios.js and .android.js files
