...
function shouldEmbedAssetsForExpoUpdates(projectRoot, exp, pkg, target) {
  ...
  if (!((_pkg$dependencies = pkg.dependencies) === null || _pkg$dependencies === void 0 ? void 0 : _pkg$dependencies['expo-updates']) || target !== 'bare') {
    return false;
...
async function _maybeConfigureExpoUpdatesEmbeddedAssetsAsync(config) {
  ...
  if (!((_config$pkg$dependenc = config.pkg.dependencies) === null || _config$pkg$dependenc === void 0 ? void 0 : _config$pkg$dependenc['expo-updates']) || config.target === 'managed') {
    return;
  }
  ...
  if (isLikelyFirstPublish) {
    _Logger().default.global.warn('🚀 It looks like this your first publish for this project! ' + "We've automatically set some configuration values in Expo.plist and AndroidManifest.xml. " + "You'll need to make a new build with these changes before you can download the update " + 'you just published.');
  }
}
...
function getIOSPaths(projectRoot) {
  ...
  const iosSupportingDirectory = _path().default.join(projectRoot, 'ios', sanitizedName(projectName), 'Supporting');
