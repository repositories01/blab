...
async function _getFirstAvailableDeviceAsync() {
  ...
  const iOSRuntimesNewestToOldest = Object.keys(simulatorDeviceInfo).filter(runtime => runtime.includes('iOS')).reverse();
...
async function _isExpoAppInstalledOnCurrentBootedSimulatorAsync() {
  ...
  const matches = (0, _glob().sync)('data/Containers/Data/Application/**/Snapshots/host.exp.Exponent{,**}', {
    cwd: simDir
  });
...
async function _expoVersionOnCurrentBootedSimulatorAsync() {
  ...
  const simDir = await _dirForSimulatorDevice(device.udid);
  const matches = (0, _glob().sync)('data/Containers/Bundle/Application/*/Exponent-*.app', {
    cwd: simDir
  });
...
async function _downloadSimulatorAppAsync(url, downloadProgressCallback) {
  ...
  const dir = _path().default.join(_simulatorCacheDirectory(), `${filename}.app`);
