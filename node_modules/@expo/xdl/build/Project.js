...
async function exportForAppHosting(projectRoot, publicUrl, assetUrl, outputDir, options = {}) {
  ...
  const {
    assets
  } = await exportAssetsAsync(projectRoot, exp, publicUrl, outputDir, bundles);
...
async function _saveAssetsAsync(projectRoot, assets, outputDir) {
...
async function _collectAssets(projectRoot, exp, hostedAssetPrefix, bundles) {
...
async function publishAssetsAsync(projectRoot, exp, bundles) {
...
async function exportAssetsAsync(projectRoot, exp, hostedUrl, outputDir, bundles) {
  _Logger().default.global.info('Analyzing assets');
  ...
  return async (req, res) => {
    try {
      ...
      if (req.headers['exponent-accept-signature']) {
        manifestString = !currentSession || _Config().default.offline ? getUnsignedManifestString(manifest) : await getSignedManifestStringAsync(manifest, currentSession);
      } else {
      ...
      Analytics().logEvent('Serve Manifest', {
        projectRoot,
        developerTool: _Config().default.developerTool,
        sdkVersion: (_manifest$sdkVersion = manifest.sdkVersion) !== null && _manifest$sdkVersion !== void 0 ? _manifest$sdkVersion : null
      });
    } catch (e) {
