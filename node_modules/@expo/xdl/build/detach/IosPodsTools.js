...
function _validatePodfileSubstitutions(substitutions) {
  const validKeys = [// a pod dependency on ExpoKit (can be local or remote)
  'EXPOKIT_DEPENDENCY', // local path to ExpoKit dependency
  'EXPOKIT_PATH', // tag to use for ExpoKit dependency
  'EXPOKIT_TAG', // the contents of dependencies.json enumerated as deps in podfile format
  'EXPONENT_CLIENT_DEPS', // postinstall for detached projects (defines EX_DETACHED among other things)
  'PODFILE_DETACHED_POSTINSTALL', // same as previous but also defines EX_DETACHED_SERVICE
  'PODFILE_DETACHED_SERVICE_POSTINSTALL', // ExponentIntegrationTests
  'PODFILE_TEST_TARGET', // unversioned react native pod dependency, probably at the path given in
...
function _renderExpoKitDependency(options, sdkVersion) {
  ...
  if (options.expoKitPath) {
    attributes = {
      path: _path().default.join(options.expoKitPath, 'ios')
    };
...
${(0, _indentString().default)(_renderDependencyAttributes(attributes), 2)}`;
  return (0, _indentString().default)(dependency, 2);
}
/**
...
function _renderUnversionedReactDependency(options, sdkVersion) {
  ...
  const attributes = {
    ...
    subspecs: ['Core', 'ART', 'RCTActionSheet', 'RCTAnimation', 'RCTCameraRoll', 'RCTGeolocation', 'RCTImage', 'RCTNetwork', 'RCTText', 'RCTVibration', 'RCTWebSocket', 'DevSupport', 'CxxBridge']
  };
  return `pod 'React',
${(0, _indentString().default)(_renderDependencyAttributes(attributes), 2)}`;
}
...
async function _renderVersionedReactNativeDependenciesAsync(templatesDirectory, versionedReactNativePath, expoSubspecs, shellAppSdkVersion) {
  const filterFn = createSdkFilterFn(shellAppSdkVersion);
  let result = await _concatTemplateFilesInDirectoryAsync(_path().default.join(templatesDirectory, 'versioned-react-native', 'dependencies'), filterFn);
...
async function _renderVersionedReactNativePostinstallsAsync(templatesDirectory, shellAppSdkVersion) {
  const filterFn = createSdkFilterFn(shellAppSdkVersion);
...
async function _concatTemplateFilesInDirectoryAsync(directory, filterFn) {
  const templateFilenames = (0, _glob().sync)('*.rb', {
    absolute: true,
    cwd: directory
  }).sort();
  const filteredTemplateFilenames = filterFn ? templateFilenames.filter(filterFn) : templateFilenames;
  const templateStrings = []; // perform this in series in order to get deterministic output
  ...
  for (let fileIdx = 0, nFiles = filteredTemplateFilenames.length; fileIdx < nFiles; fileIdx++) {
    ...
    const templateString = await _fsExtra().default.readFile(filename, 'utf8');
...
function _renderDetachedPostinstall(sdkVersion, isServiceContext) {
  ...
  const maybeFrameworkSearchPathDef = sdkMajorVersion < 33 ? `
...
function _renderUnversionedPostinstall(sdkVersion) {
  ...
  const gccPreprocessorDefinitionsCondition = sdkMajorVersion < 36 ? `${podNameExpression} == 'React'` : `${podNameExpression}.start_with?('React')`;
  return `
...
async function _renderPodDependenciesAsync(dependenciesConfigPath, options) {
  const dependencies = await new (_jsonFile().default)(dependenciesConfigPath).readAsync();
...
async function renderExpoKitPodspecAsync(pathToTemplate, pathToOutput, moreSubstitutions) {
  const templatesDirectory = _path().default.dirname(pathToTemplate);

  const templateString = await _fsExtra().default.readFile(pathToTemplate, 'utf8');
  const dependencies = await _renderPodDependenciesAsync(_path().default.join(templatesDirectory, 'dependencies.json'), {
    isPodfile: false
  });
...
async function renderPodfileAsync(pathToTemplate, pathToOutput, moreSubstitutions, shellAppSdkVersion, sdkVersion = 'UNVERSIONED') {
  ...
  const versionedDependencies = await _renderVersionedReactNativeDependenciesAsync(templatesDirectory, versionedRnPath, rnExpoSubspecs, shellAppSdkVersion);
  const versionedPostinstalls = await _renderVersionedReactNativePostinstallsAsync(templatesDirectory, shellAppSdkVersion);
  const podDependencies = await _renderPodDependenciesAsync(_path().default.join(templatesDirectory, 'dependencies.json'), {
    isPodfile: true
  });
