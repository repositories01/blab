...
async function action(projectDir, command) {
  ...
  if (projectDir) {
    const root = _path().default.resolve(projectDir);
  ...
  let initialConfig;
  const templateManifest = await _pacote().default.manifest(templateSpec);
  ...
  if (isBare) {
    ...
    const showPublishBeforeBuildWarning = await (0, _ProjectUtils().usesOldExpoUpdatesAsync)(projectPath);
...
async function configureUpdatesProjectFilesAsync(projectRoot, initialConfig, username) {
  const {
    exp
  } = await (0, _config().getConfig)(projectRoot); // apply Android config
  ...
  const supportingDirectory = _path().default.join(projectRoot, 'ios', initialConfig.name, 'Supporting');
...
function validateName(parentDir, name) {
  ...
  const dir = _path().default.join(parentDir, name);
