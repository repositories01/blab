...
function _default(program) {
  program.command('webhooks [project-dir]').description('List all webhooks for a project.').asyncActionProjectDir(listAsync);
  program.command('webhooks:add [project-dir]').description('Add webhook to a project.').option('--url <url>', 'URL to request. (Required)').option('--event <event-type>', 'Event type that triggers the webhook. [build] (Required)').option('--secret <secret>', "Secret used to create a hash signature of the request payload, provided in the 'Expo-Signature' header.").asyncActionProjectDir(addAsync);
  program.command('webhooks:remove [project-dir]').option('--id <id>', 'ID of the webhook to remove.').description('Delete a webhook.').asyncActionProjectDir(removeAsync);
  program.command('webhooks:update [project-dir]').option('--id <id>', 'ID of the webhook to update.').option('--url [url]', 'URL the webhook will request.').option('--event [event-type]', 'Event type that triggers the webhook. [build]').option('--secret [secret]', "Secret used to create a hash signature of the request payload, provided in the 'Expo-Signature' header.").description('Update a webhook for a project.').asyncActionProjectDir(updateAsync);
}
