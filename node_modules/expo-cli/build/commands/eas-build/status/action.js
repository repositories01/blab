...
function printBuildTable(builds) {
  ...
  const refactoredBuilds = builds.map(build => {
    ...
    const buildUrl = (_build$artifacts = build.artifacts) === null || _build$artifacts === void 0 ? void 0 : _build$artifacts.buildUrl;
    return {
      ...
      artifact: buildUrl ? // Trim the URL here, otherwise if printTableJsonArray trims it, it incorrectly removes the escape to end the link
      // Which makes everything in the terminal a link after printing the table
      _log().default.terminalLink(buildUrl.length > 38 ? `${buildUrl.slice(0, 38)}…` : buildUrl, buildUrl) : 'not available'
    };
  });
