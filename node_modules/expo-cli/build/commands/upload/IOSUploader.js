...
class IOSUploader extends _BaseUploader().default {
  ...
  async _getAppleTeamId(appleIdCrentials) {
    ...
    const teamId = credential === null || credential === void 0 ? void 0 : credential.teamId;
  ...
  async _getAppleIdCredentials() {
    var _result$appleId, _ref, _result$appleIdPasswo;
    ...
    if (result.appleId && process.env.EXPO_APPLE_ID) {
      _log().default.warn(`You've provided contradictory Apple IDs. You should provide either the EXPO_APPLE_ID env or the --apple-id flag, not both. Falling back to --apple-id.`);
    }

    if (result.appleIdPassword && process.env.EXPO_APPLE_PASSWORD) {
      _log().default.warn(`You've provided contradictory Apple passwords. You should provide either the EXPO_APPLE_PASSWORD env or the --apple-id-password flag, not both. Falling back to --apple-id-password.`);
    }

    const appleId = (_result$appleId = result.appleId) !== null && _result$appleId !== void 0 ? _result$appleId : process.env.EXPO_APPLE_ID;
    const appleIdPassword = (_ref = (_result$appleIdPasswo = result.appleIdPassword) !== null && _result$appleIdPasswo !== void 0 ? _result$appleIdPasswo : process.env.EXPO_APPLE_PASSWORD) !== null && _ref !== void 0 ? _ref : process.env.EXPO_APPLE_ID_PASSWORD;
