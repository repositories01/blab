...
async function upgradeAsync({
  ...
  if (targetSdkVersionString === latestSdkVersionString && currentSdkVersionString !== targetSdkVersionString && !_commander().default.nonInteractive) {
    ...
    if (!answer.updateToLatestSdkVersion) {
      const sdkVersionStringOptions = Object.keys(sdkVersions).filter(v => _semver().default.lte('33.0.0', v) && !_xdl().Versions.gteSdkVersion(exp, v));
      const {
  ...
  installingPackageStep.succeed(`Installed ${expoPackageToInstall}`); // Evaluate project config (app.config.js)
