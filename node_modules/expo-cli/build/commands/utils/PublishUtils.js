...
async function getPublishHistoryAsync(projectDir, options) {
...
async function setPublishToChannelAsync(projectDir, options) {
  const user = await _xdl().UserManager.ensureLoggedInAsync();
...
async function getPublicationDetailAsync(projectDir, options) {
  // TODO(ville): handle the API result for not authenticated user instead of checking upfront
  const user = await _xdl().UserManager.ensureLoggedInAsync();
  const {
    exp
