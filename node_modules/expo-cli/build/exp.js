...
function runAsync(programName) {
  try {
    ...
    _commander().default.version(packageJSON.version).option('--non-interactive', 'Fail, if an interactive prompt would be required to continue. Enabled by default if stdin is not a TTY.'); // Load each module found in ./commands by 'registering' it with our commander instance
...
function generateCommandJSON() {
  _commander().default.name('expo');

  (0, _commands().registerCommands)(_commander().default);
  return _commander().default.commands.map(commandAsJSON);
} // The type definition for Option seems to be wrong - doesn't include defaultValue


function optionAsJSON(option) {
  return {
    flags: option.flags,
...
function commandAsJSON(command) {
  return {
    name: command.name(),
...
function formatOptionAsMarkdown(option) {
  return `| \`${sanitizeFlags(option.flags)}\` | ${option.description} |`;
}

function formatOptionsAsMarkdown(options) {
...
function formatCommandAsMarkdown(command) {
  return `
...
function formatCommandsAsMarkdown(commands) {
  ...
  (async function () {
    if (process.argv[2] === 'introspect') {
      ...
      if (process.argv[3] && process.argv[3].includes('markdown')) {
        (0, _log().default)(formatCommandsAsMarkdown(commands));
      } else {
        (0, _log().default)(JSON.stringify(commands));
      }
    } else {
